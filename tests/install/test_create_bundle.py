...
class TestChecksumAssetSelection:
    def test_a_per_asset_sidecar_wins_over_a_combined_file(self):
        ...
        assert create_bundle.select_checksum_asset(names, 'tool.tar.gz') == 'tool.tar.gz.sha256'

    def test_a_combined_file_is_the_fallback(self):
        names = ['tool.tar.gz', 'checksums.txt']
        assert create_bundle.select_checksum_asset(names, 'tool.tar.gz') == 'checksums.txt'
        assert create_bundle.select_checksum_asset(['SHA256SUMS'], 'tool.tar.gz') == 'SHA256SUMS'
    ...
    def test_signatures_and_certificates_are_never_mistaken_for_checksums(self):
        ...
        assert create_bundle.select_checksum_asset(names, 'tool.zip') == 'checksums.txt'

        for aux in ['checksums.txt.sig', 'checksums.txt.asc', 'checksums.json', 'checksums.txt-bsd']:
            assert create_bundle.select_checksum_asset([aux], 'tool.zip') is None

    def test_a_release_publishing_nothing_usable_returns_none(self):
        assert create_bundle.select_checksum_asset(['tool.tar.gz', 'README.md'], 'tool.tar.gz') is None
        assert create_bundle.select_checksum_asset([], 'tool.tar.gz') is None


class TestChecksumLookup:
    def test_the_sha256sum_format_including_the_binary_marker(self):
        text = 'abc123  other.tar.gz\ndef456  tool.tar.gz\n'
        assert create_bundle.checksum_for_asset(text, 'tool.tar.gz') == 'def456'
        assert create_bundle.checksum_for_asset('def456 *tool.tar.gz\n', 'tool.tar.gz') == 'def456'
    ...
    def test_a_leading_path_is_stripped_only_when_nothing_matched_exactly(self):
        ...
        assert create_bundle.checksum_for_asset(text, 'tool.tar.gz') == 'bbb'
    ...
    def test_case_differences_are_accepted_because_github_resolves_them(self):
        ...
        assert create_bundle.checksum_for_asset(text, 'lazygit_0.44_Linux_x86_64.tar.gz') == 'def456'

    def test_a_bare_digest_counts_only_in_a_file_containing_nothing_else(self):
        digest = 'a' * 64
        assert create_bundle.checksum_for_asset(f'{digest}\n', 'tool.tar.gz') == digest
    ...
    def test_an_unreadable_table_yields_nothing_rather_than_a_wrong_digest(self):
        ...
        assert create_bundle.checksum_for_asset('yq_linux_amd64 MD5:abc SHA256:def\n', 'yq_linux_amd64') is None
...
class TestReleaseUrl:
    def test_repo_and_tag_are_recovered_including_a_tag_with_slashes(self):
        assert create_bundle.parse_release_url('https://github.com/sharkdp/fd/releases/download/v10.2.0/fd.tar.gz') == (
            'sharkdp/fd',
            'v10.2.0',
        )
        assert create_bundle.parse_release_url('https://github.com/o/r/releases/download/cli/v1.0/x.tar.gz') == ('o/r', 'cli/v1.0')
    ...
    def test_anything_that_is_not_a_github_release_is_rejected(self):
        ...
        assert create_bundle.parse_release_url('https://releases.hashicorp.com/terraform/1.0/terraform.zip') is None
        assert create_bundle.parse_release_url('https://astral.sh/uv/install.sh') is None
