...
@test "glow: installs successfully with real network call" {
  run docker_exec "$BATS_SHARED_CONTAINER" \
    "bash install/common/github-releases/glow.sh"
  assert_success
...
@test "lazygit: installed binary is executable after installation" {
  run docker_exec "$BATS_SHARED_CONTAINER" \
    "bash install/common/github-releases/lazygit.sh && ~/.local/bin/lazygit --version"
  assert_success
}

@test "duf: can be installed and run in isolated container" {
  run docker_exec "$BATS_SHARED_CONTAINER" \
    "bash install/common/github-releases/duf.sh && ~/.local/bin/duf --version"
  assert_success
}
