...
FAILURES_LOG="/tmp/test-github-releases-$$.log"
export FAILURES_LOG
rm -f "$FAILURES_LOG"

# Create mock GitHub release installer that uses new pattern
MOCK_INSTALLER="/tmp/mock-github-release.sh"
...
# Return error code (don't exit - let wrapper handle it)
exit 1
EOF
...
if echo "$OUTPUT" | grep -q "FAILURE_TOOL='mock-tool'"; then
  pass "Outputs FAILURE_TOOL field"
else
  fail "Missing FAILURE_TOOL field"
fi

if echo "$OUTPUT" | grep -q "FAILURE_URL='https://github.com/mock/tool"; then
...
if grep -q "mock-tool - Installation Failed" "$FAILURES_LOG"; then
  pass "Log contains tool name"
else
  fail "Log missing tool name"
fi

if grep -q "Download URL: https://github.com/mock/tool" "$FAILURES_LOG"; then
