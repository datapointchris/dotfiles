...
def test_a_bare_pin_matches_whichever_spelling_the_repo_publishes(github, tags, pin, prefix, resolves_to):
    github.serve(RELEASE_LIST, published(*tags))

    assert github_release.tag_for_version('owner/repo', pin, prefix) == resolves_to


def test_a_pin_nothing_published_refuses_instead_of_naming_the_newest(github):
...
def test_a_draft_never_satisfies_a_pin(github):
...
def test_the_pin_is_matched_against_one_listing_rather_than_asked_for_by_name(github):
...
def test_a_pin_older_than_one_page_of_releases_is_refused_like_one_nothing_published(github):
...
def test_every_published_asset_is_named_with_the_id_the_asset_endpoint_wants(github):
    github.serve(
...
def test_a_tag_carrying_a_slash_is_encoded_into_one_path_segment(github):
...
def test_a_release_publishing_nothing_answers_an_empty_mapping(github, body):
...
@unreadable
def test_an_unreadable_release_answers_nothing_rather_than_raising(monkeypatch, transport):
    monkeypatch.setattr(github_release, 'request', transport)
...
@unreadable
def test_an_unreadable_release_is_told_apart_from_one_that_publishes_no_checksums(tmp_path, monkeypatch, transport):
...
def test_a_matching_digest_exits_verified_and_prints_nothing(tmp_path, capsys):
    payload = downloaded(tmp_path)
...
def test_a_tampered_file_exits_failed_and_is_deleted(tmp_path):
    payload = downloaded(tmp_path)
...
def test_a_source_that_names_no_release_exits_unpublished(tmp_path, source):
...
def test_the_repo_and_tag_positionals_reach_the_release_they_name(tmp_path, github, no_github_credential):
...
def test_checksum_url_names_the_file_directly_for_a_release_github_does_not_host(tmp_path, github):
    ...
    code = github_release.main(['verify', str(payload), ASSET, '--checksum-url', 'https://releases.hashicorp.com/demo/1.0/demo_SHA256SUMS'])

    assert code == github_release.Verification.VERIFIED
    assert github.asked == ['https://releases.hashicorp.com/demo/1.0/demo_SHA256SUMS']


def test_a_checksum_url_that_will_not_answer_exits_failed(tmp_path, github):
...
def test_parse_url_prints_the_pair_or_nothing_and_succeeds_either_way(capsys, url, printed):
...
def test_checksum_for_prints_the_digest_or_nothing_and_succeeds_either_way(tmp_path, capsys, asset, printed):
...
def test_sha256_prints_the_digest_and_nothing_else(tmp_path, capsys):
...
def test_a_call_the_door_does_not_have_refuses_rather_than_guessing(capsys, argv):
...
def run(tmp_path: Path, *argv: str) -> subprocess.CompletedProcess[str]:
...
def test_the_module_runs_by_hand_and_prints_the_digest(tmp_path):
    answered = run(tmp_path, 'sha256', str(downloaded(tmp_path)))

    assert answered.returncode == 0
    assert answered.stdout == f'{DIGEST}\n'


def test_a_failed_verification_leaves_the_shell_a_non_zero_status(tmp_path):
    payload = downloaded(tmp_path)
    recorded = checksums_file(tmp_path)
    payload.write_bytes(b'not what was digested')

    answered = run(tmp_path, 'verify', str(payload), ASSET, '--bundle-checksums', str(recorded))
