...
The base image defaults to Ubuntu 26.04, matching the work WSL. Build another
with `./build-base.sh --ubuntu 24.04`, which tags it separately.
...
./run-installer-test.sh install/common/github-releases/lazygit.sh

# Test system packages (should skip - already installed in base)
./run-installer-test.sh install/wsl/system-packages.sh

# Keep container for debugging
./run-installer-test.sh install/common/github-releases/lazygit.sh --keep
```
...
```bash
$ time ./run-installer-test.sh install/common/github-releases/glow.sh
