...
def test_relink_never_unlinks_a_config_a_live_daemon_is_watching(tmp_path, monkeypatch):
    """Hyprland reloads the moment its config changes, and writes itself a
    default when it finds none. A relink that removed every link before
    recreating them handed it exactly that window, and the create pass then
    refused the default as a target it had not made — so the run failed and the
    daemon kept running the stub. The stand-in below writes that stub if the
    deployed link is ever absent when the run's first step executes."""
    dotfiles = tmp_path / 'dotfiles'
    home = tmp_path / 'home'
    hypr = dotfiles / 'configs' / 'archlinux' / '.config' / 'hypr'
    hypr.mkdir(parents=True)
    (hypr / 'hyprland.conf').write_text('source = conf/keybindings.conf')
    (dotfiles / 'configs' / 'common').mkdir(parents=True)
    home.mkdir()

    deployed = home / '.config' / 'hypr' / 'hyprland.conf'
    core.create_symlinks(dotfiles / 'configs' / 'archlinux', 'archlinux', target_dir=home)
    assert deployed.is_symlink()

    prune = core.check_and_clean

    def daemon_regenerates_when_config_vanishes(*args, **kwargs):
        if not deployed.exists():
            deployed.parent.mkdir(parents=True, exist_ok=True)
            deployed.write_text('autogenerated = 1')
        return prune(*args, **kwargs)

    monkeypatch.setattr(core, 'check_and_clean', daemon_regenerates_when_config_vanishes)

    refused = core.relink('archlinux', dotfiles_dir=dotfiles, target_dir=home)
