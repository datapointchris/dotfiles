...
def test_a_batched_resource_is_handed_its_whole_run_at_once(session: Session, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # A Refusal is an Exception and carries no `change`, so reading it off the
    # payload directly raised AttributeError where the batch had failed — the one
    # case the item order below is here to prove did not happen.
