...
variant's `repos_registry` points at is hand-edited and generated by nothing, and
...
This box replicates nothing between machines. Every directory the fleet shares
that way is absent here. A tool depending on one is either off the manifest or
configured differently, which is a property of the machine rather than something
the install detects.
