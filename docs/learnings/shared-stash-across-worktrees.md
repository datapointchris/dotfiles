...
under `~/.dotfiles-wt/`, precisely so a branch cannot deploy itself over the machine, so two or
three simultaneous worktrees is the normal state rather than an unusual one.
