...
├── shell/               # Shell source files (symlinked to ~/.local/shell/)
│   ├── functions.sh     # Shell functions (cross-platform)
│   ├── aliases.sh       # Shell aliases (cross-platform)
│   ├── macos.sh         # macOS functions + aliases
│   ├── arch.sh          # Arch Linux functions + aliases
│   └── wsl.sh           # WSL functions + aliases
...
4. Links shell source files from `shell/` to `~/.local/shell/` (common: functions.sh + aliases.sh; platform: {platform}.sh)
...
Shell functions and aliases live in `shell/` as flat source files deployed directly via symlinks — no build step required.

- **Cross-platform**: `shell/functions.sh` and `shell/aliases.sh` → `~/.local/shell/`
- **Platform-specific**: `shell/{platform}.sh` (macos, arch, wsl) → `~/.local/shell/{platform}.sh`
