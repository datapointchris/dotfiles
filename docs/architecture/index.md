...
### Three places a machine can answer, not one
...
So a declared name resolves through three rungs, highest first: `$DOTFILES_REPOS_JSON` for this tool alone, `$REPOS_JSON` for every reader of the same file, then `repos_file` in `~/.config/dotfiles/config.toml`. Each answers a different question — this invocation, this machine's shells, this machine — and the config file is the only one a process with no shell can read. `src/dotfiles/settings.py` is the resolver and its module docstring carries the reasoning; `standards/data.md` § "A shared file is named in config; only the tool's own default is compiled in" is the rule it implements.

There is deliberately no fourth rung. A compiled-in default naming a path outside this tool's own XDG directories is what that standard forbids, and the registry such a default would have to guess at moved twice in six hours on the day this was written. Unset everywhere stays unset, and the check says so rather than resolving to something plausible.

That split is also why a missing file now reports two different findings. *Nothing names its location* is a machine that never answered, and the advice names all three places; *absent at a path, from a named rung* is a machine that answered and has no file there, which is the state between an install and safekeep's restore. Only the second is something safekeep can fix, and reporting both as the first is what made the timer's advice wrong.
