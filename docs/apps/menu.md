...
`apps/common/menu-dashboard` (the cross-app glance). All three share two small repo-root modules
imported via each script's resolved path, so no install step is needed for them: `menucore` (cadence
parsing, atomic state, the nudge renderers), which keeps the register and Labs from drifting apart
on scheduling, and `apppaths` (XDG path resolution), which every Python app in `apps/` shares. The
color palette and help grammar are no longer repo-local — they come from the `pytermstyle` package,
declared in each script's PEP 723 header — see
