...
- **Install** — `packages.yml` under `git_uv_tools`, installed by `install/common/language-tools/uv-tools.sh` with `uv tool install <repo url>`. The entry point lands in `~/.local/bin`. Update with `refcheck --update`.
