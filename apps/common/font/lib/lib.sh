...
# Get current font size from Ghostty fonts config
get_font_size() {
  local config="$HOME/.config/ghostty/fonts/current.conf"
  if [[ -f "$config" ]]; then
    grep "^font-size" "$config" 2>/dev/null | cut -d= -f2 | xargs
  fi
}

# Apply font to Ghostty (writes to fonts/current.conf)
apply_font_ghostty() {
  local font="$1"
  local config_dir="$HOME/.config/ghostty/fonts"
  local config="$config_dir/current.conf"

  mkdir -p "$config_dir"

  # Preserve current font size or use default
  local size
  size=$(get_font_size)
  [[ -z "$size" ]] && size=17

  cat > "$config" << EOF
# Generated by 'font apply' - do not edit manually
font-family = "${font}"
font-size = ${size}
EOF
  return 0
...
# Apply font to Kitty (writes to fonts/current.conf)
apply_font_kitty() {
  local font="$1"
  local config_dir="$HOME/.config/kitty/fonts"
  local config="$config_dir/current.conf"
