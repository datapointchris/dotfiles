...
Every observation is unprivileged and every repair needs root, which is the whole
reason this subsystem can be *checked* for the first time. `install/archlinux/
system-config.sh` was already written as check-then-act — `getent group docker`,
`systemctl is-enabled`, `grep -q autologin` — it simply had no way to show anyone
its plan, and its reads were interleaved with its writes so running it at all
meant a password.

Nothing here says `sudo`. A write takes an authorized `Privilege` and refuses
without one, so a machine with no root still converges everything else and says
...
class Result:
    ...
    run. It is the same distinction `pluginsync.blocked` draws for TPM, reached
    here for the same reason — every one of these preconditions is supplied by an
    earlier stage of the same run.
    """
...
def apply(entry: catalog.SystemConfig, privilege: Privilege) -> Result:
    try:
        if isinstance(entry, catalog.GroupMembership):
            return _apply_group(entry, privilege)
        if isinstance(entry, catalog.SystemdUnit):
...
def _apply_group(entry: catalog.GroupMembership, privilege: Privilege) -> Result:
    user = current_user()
    try:
        ...
        created = privilege.run(['groupadd', entry.name], reason=f'create the {entry.name} group')
        if not created.ok:
            return Result(False, f'groupadd {entry.name} failed: {created.transcript.strip()}')

    added = privilege.run(['usermod', '-aG', entry.name, user], reason=f'add {user} to the {entry.name} group')
    if not added.ok:
...
def _apply_unit(entry: catalog.SystemdUnit, privilege: Privilege) -> Result:
    verb = 'enable' if entry.enabled else 'disable'
    changed = privilege.run(['systemctl', verb, entry.name], reason=f'{verb} {entry.name}')
    if not changed.ok:
    ...
    started = privilege.run(['systemctl', 'start', entry.name], reason=f'start {entry.name}')
    if not started.ok:
...
def _apply_file(entry: catalog.ManagedFile, privilege: Privilege) -> Result:
    ...
    the file exists with the wrong mode. The bash it replaces needed `sudo mkdir`
    and `sudo tee` and a heredoc whose backslashes had to survive two levels of
    expansion.
    """
    path = target_path(entry)
    content = _wanted_content(entry, path)

    staged = Path(tempfile.mkstemp(prefix='dotfiles-managed-')[1])
    try:
        staged.write_text(content)
        installed = privilege.run(
            ['install', '-D', '-m', entry.mode, str(staged), str(path)],
            reason=f'write {path}',
        )
    finally:
...
def _observe_login_shell(entry: catalog.LoginShell) -> State:
    ...
    It used to answer `Repair.NONE` when the shell was not on PATH, which reads a
    fact that this same run supplies: zsh is a system package at `SYSTEM` and this
    row is decided at `SYSTEM_CONFIG`. Measured before either has run — which is
    every fresh machine — that made the login shell unrepairable and the install
    finished leaving bash in place. Whether the shell has arrived is the repair's
    question, asked at the moment it matters.
    """
