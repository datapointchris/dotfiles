...
def failure(name: str, completed: effects.Completed) -> str:
    ...
    **A line naming a cause is carried out of the head when the tail lost it.** curl
    ...
    a box behind an intercepting proxy, 2026-08-17: eight applies recorded a
    claude-code failure whose every line was curl's boilerplate, and the CA advice
    written for that machine never once reached it.
    """
