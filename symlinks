...
resolve_symlink_target() {
    ...
    if [[ "$target" = /* ]]; then
        ...
        # Relative path - convert to absolute without changing directories
...
remove_symlinks() {
    ...
    done < <(find "$target_dir" -maxdepth "$SEARCH_DEPTH" -type l \
        -not -path "*/Library/*" \
        -not -path "*/.Trash/*" \
...
show_symlinks() {
    ...
    if [[ "$symlinks_found" == "false" ]]; then
        ...
        log_info "Run '$0 check' to automatically remove broken symlinks"
    fi
}
...
check_dangling_symlinks() {
    ...
    # Find ALL dangling symlinks, not just dotfiles ones
    while IFS= read -r symlink_path; do
        if [[ -L "$symlink_path" ]]; then
            ...
            if [[ ! -e "$symlink_path" ]]; then
                # This symlink is broken - check if it's dotfiles-related or just broken
                local is_dotfiles_related=false

                # Check if the target path contains 'dotfiles' anywhere in the path
                if [[ "$target" == *"dotfiles"* ]]; then
                    is_dotfiles_related=true
                fi

                # Also check if it points to a resolved path containing dotfiles
                local symlink_dir
                ...
                if [[ -n "$absolute_target" && "$absolute_target" == *"dotfiles"* ]]; then
                    is_dotfiles_related=true
                fi

                # Include all broken symlinks in critical directories like nvim plugins
                if [[ "$symlink_path" == *".config/nvim"* ]]; then
                    is_dotfiles_related=true
                fi

                if [[ "$is_dotfiles_related" == true ]]; then
                    dangling_links+=("$symlink_path")
                    link_count=$((link_count + 1))
                fi
            fi
        fi
    done < <(find "$search_path" -maxdepth "$SEARCH_DEPTH" -type l \
        -not -path "*/Library/*" \
        -not -path "*/.Trash/*" \
        -not -path "*/node_modules/*" \
        -not -path "*/.git/*" \
...
show_usage() {
    ...
    echo "  check            - Check for and automatically remove ALL broken symlinks"
    echo "                     (scans entire \$HOME)"
    ...
    echo "  $0 check                     # Check for and remove ALL broken symlinks"
    echo ""
    echo "Examples:"
    ...
    echo "  $0 check                     # Check entire \$HOME for broken symlinks (auto-removes them)"
    echo ""
    ...
    echo "NOTE: Relink includes comprehensive broken symlink detection and cleanup."
    echo "NOTE: The 'show' command highlights broken symlinks in red."
    echo "NOTE: The 'check' command automatically removes broken symlinks."
    echo ""
    echo "TROUBLESHOOTING:"
    echo "  If you see Neovim errors like 'Failed to load plugins.X', run:"
    echo "  $0 check                     # This will find and remove broken symlinks"
    echo "                               # like ~/.config/nvim/lua/plugins/native-lsp.lua"
}
...
main() {
    ...
    # Handle standalone check command
    if [[ "$action" == "check" ]]; then
        if [[ -n "$target" ]]; then
            log_warning "Note: The 'check' command now scans the entire \$HOME directory for comprehensive broken symlink detection."
        fi
        check_dangling_symlinks
        return 0
    fi
