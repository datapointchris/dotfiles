...
source "$SCRIPT_DIR/test-install-helpers.sh"
...
STEP_START=$(date +%s)
{
  ...
  sudo -u "$TEST_USER" bash -c "
    ...
    zsh -c \"source \$ZSHDOTDIR/.zshrc 2>/dev/null; bash --norc /Users/$TEST_USER/dotfiles/management/verify-installation.sh\"
...
STEP_START=$(date +%s)
{
  ...
  sudo -u "$TEST_USER" bash "/Users/${TEST_USER}/dotfiles/management/detect-alternate-installations.sh"
} 2>&1 | tee -a "$LOG_FILE"
STEP_END=$(date +%s)
STEP_ELAPSED=$((STEP_END - STEP_START))
