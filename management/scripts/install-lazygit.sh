...
# Download URL
LAZYGIT_URL="https://github.com/${REPO}/releases/download/v${LAZYGIT_VERSION}/lazygit_${LAZYGIT_VERSION}_linux_x86_64.tar.gz"
LAZYGIT_TARBALL="/tmp/lazygit.tar.gz"

# Download
if ! download_file "$LAZYGIT_URL" "$LAZYGIT_TARBALL" "lazygit"; then
  print_manual_install "lazygit" "$LAZYGIT_URL" "v$LAZYGIT_VERSION" "lazygit_${LAZYGIT_VERSION}_linux_x86_64.tar.gz" \
    "tar -xzf ~/Downloads/lazygit_${LAZYGIT_VERSION}_linux_x86_64.tar.gz -C /tmp && mv /tmp/lazygit ~/.local/bin/ && chmod +x ~/.local/bin/lazygit"
  exit 1
fi
