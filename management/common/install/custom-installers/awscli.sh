...
case $PLATFORM in
  Linux)
    ...
    if ! curl -fsSL "$ZIP_URL" -o "$ZIP_FILE"; then
      log_warning "AWS CLI download failed"
      log_info "Download manually: $ZIP_URL"
      exit 1
    fi
    ...
    if ! "$EXTRACT_DIR/aws/install" --install-dir "$HOME/.local/aws-cli" --bin-dir "$HOME/.local/bin" --update; then
      ...
      log_info "See: https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html"
      exit 1
    fi
...
if command -v aws >/dev/null 2>&1; then
  ...
  log_warning "AWS CLI not found in PATH after installation"
  log_info "Ensure ~/.local/bin is in PATH: export PATH=\"\$HOME/.local/bin:\$PATH\""
  exit 1
fi
