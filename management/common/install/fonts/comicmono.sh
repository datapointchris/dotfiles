...
download_comicmono() {
  ...
  for file in "${files[@]}"; do
    if ! curl -fsSL "$base_url/$file" -o "$download_dir/$file"; then
      manual_steps="Download manually from website:
   ${base_url}/${file}
...
Save files to:
   $system_font_dir/"
      ...
      output_failure_data "ComicMono" "${base_url}/${file}" "latest" "$manual_steps" "Download failed for $file"
      exit 1
    fi
  done
...
if is_font_installed "$system_font_dir" "*ComicMono*.$font_extension"; then
