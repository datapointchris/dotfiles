...
get_download_url() {
  ...
  local platform_arch
  if [[ "$os" == "darwin" ]]; then
    [[ "$arch" == "arm64" ]] && platform_arch="darwin.aarch64" || platform_arch="darwin.x86_64"
  else
    platform_arch="linux.x86_64"
  fi
  echo "https://github.com/${REPO}/releases/download/${version}/shellcheck-${version}.${platform_arch}.tar.xz"
}
