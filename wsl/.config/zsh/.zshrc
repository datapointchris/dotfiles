...
# ------------------------------------------------------------------- #
# PATH AND ENVIRONMENT SETUP (WSL-specific)
# ------------------------------------------------------------------- #
function add_path() {
    if [[ -d "$1" ]]; then
        export PATH="$1:$PATH"
...
# WSL-specific: Source cargo environment (installed via rustup)
if [[ -f "$HOME/.cargo/env" ]]; then
    source "$HOME/.cargo/env"
fi

# WSL-specific: Cargo configuration with XDG compliance
...
# WSL-specific paths
add_path "/opt/nvim"                # Neovim AppImage location
add_path "/usr/local/go/bin"        # Go binary (manually installed)
add_path "/snap/bin"                # Snap packages
add_path "$HOME/.local/bin"         # Local binaries
add_path "/usr/local/bin"           # System-wide binaries
