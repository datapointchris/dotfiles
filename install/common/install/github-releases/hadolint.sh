...
get_download_url() {
  ...
  local platform_arch
  if [[ "$os" == "darwin" ]]; then
    [[ "$arch" == "arm64" ]] && platform_arch="macos-arm64" || platform_arch="macos-x86_64"
  else
    [[ "$arch" == "arm64" ]] && platform_arch="linux-arm64" || platform_arch="linux-x86_64"
  fi
  echo "https://github.com/${REPO}/releases/download/${version}/hadolint-${platform_arch}"
}
...
# hadolint releases are direct binaries (no archive)
...
2. After downloading, install:
   mv ~/Downloads/hadolint-* ~/.local/bin/hadolint
...
if command -v hadolint >/dev/null 2>&1; then
  log_success "hadolint installed to: $TARGET_BIN"
else
