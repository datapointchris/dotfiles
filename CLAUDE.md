...
fleet's data as its own responsibility. Both exist because a nonfleet box runs this same engine with
none of it.
...
`.markdownlint.yaml` are generated by forge from this repo's `toolchain.components` in
...
- **Machine-local shell code goes in `~/.local/shell/local.sh`** - Declared as a `required_files` entry in `install/flags.yml` and sourced last by `.zshrc`, but never present in this repo: it holds internal hostnames and the like. Restored by safekeep rather than installed, so it is legitimately absent between `dotfiles apply` and the restore step of a rebuild — which is what `dotfiles check` reports. A mechanism that is generic (mounting a Windows share) belongs in the coordinate layer that owns it; the values naming one organisation's hosts go in the local file, and so does any workaround only that network forces — `update-tldr` reads as a WSL function and is really a blocked-download function, which is why it sat in the layer for months
