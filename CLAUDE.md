...
- **Six axes, one hand-chosen value** - `MACHINE` is the only thing chosen by hand; it selects a manifest, and the manifest says where the machine sits on each of the six axes in `src/dotfiles/coordinates.py` — package manager, OS family, display stack, host, network trust, capacity. `dotfiles env apply` resolves that point into the `<axis>/<value>` directories it selects and writes them into `~/.env` as one `DOTFILES_LAYERS` list, which every shell sources from. Nothing is detected: a guess cannot answer whether a box is on a fleet or nonfleet network, or whether it is meant to be a workstation or a server. **Never enumerate the overlay directories in prose** — read `OVERLAY_DIRS` and the enums beside it, or run `eza -1 -D configs apps shell` for the ones that exist, because an axis earns a directory only where something actually differs along it. Why the fused `PLATFORM` string was split, and why a `MACHINE_ROLE` axis was tried and removed before it: `docs/architecture/index.md`
